...
wrapper = ClassifierWrapper(signaler=CumulativeSumSignaler(0.02,True,event_col="balance"),
                            ...
                            feature_engineer=FeatureEngineer(EventSampler(cols=["open"]),
                                                             X_pipe=X_pipe
                                                             )
                            )
wrapper.set_params(**{'signaler__cusum_threshold': 0.02, 
'model': xgboost.XGBClassifier(min_child_weight=0.05,colsample_bytree=0.1,subsample=0.7),
